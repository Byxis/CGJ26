...
public class UnitCardDisplay : BaseCardDisplay
{
    ...
    protected override void RefreshUI()
    {
        ...
        if (_cardData is UnitData unitData)
        {
            // Use Runtime Stats if available (Deck/Hand/Shop instance), else fallback to Prefab Stats (Asset inspector
            // view)
            UnitStats unit = unitData.currentStats;
            if (unit == null && unitData.unitPrefab != null)
            {
                var ctrl = unitData.unitPrefab.GetComponent<UnitController>();
                if (ctrl != null)
                    unit = ctrl.Stats;
            }
            ...
            if (cdText)
                cdText.text = unit.attackCooldown.ToString();
            if (speedText)
    ...
    protected override void HandleDrop(PointerEventData eventData)
    {
        ...
        if (droppedCardDisplay != null)
            droppedData = droppedCardDisplay.CardData;
    ...
    {
        if (_cardData is UnitData unitData && unitData.currentStats != null)
        {
            // Direct modification of Runtime Stats
            UnitStats unit = unitData.currentStats;
